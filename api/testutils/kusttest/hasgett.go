...
// It registers the flag lazily on first call to avoid using init() function.
func isUpdateGolden() bool {
	f := flag.Lookup(updateGoldenFlagName)
	if f == nil {
		// Register flag lazily if not already registered
		flag.Bool(updateGoldenFlagName, false, "update golden files for krusty tests")
		f = flag.Lookup(updateGoldenFlagName)
	}
